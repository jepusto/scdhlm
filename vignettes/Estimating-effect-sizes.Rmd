...
summary(hlm1)
```
The design-comparable standardized mean difference corresponds to the treatment effect at week $B = 9$, after $B - A = 7$ weeks of treatment. The corresponding values of `p_const` and `r_const` are specified below.[^pconst]
```{r Schutte-ES-Model3, eval = TRUE}
Schutte_g1 <- g_mlm(hlm1, p_const = c(0,0,1, B - A - 1), r_const = c(1,0,1))
Schutte_g1
```
...
[^pconst]: The coding of the `p_const` argument differs from that given in Pustejovsky, Hedges, & Shadish (2014) due to a small difference in the parameterization of the treatment-by-time interaction. In the original paper, this term was computed so that the coefficient on the treatment indicator corresponds to the _instantaneous_ change in level upon introduction of the treatment. In the `scdhlm` package, the treatment-by-time interaction is coded so that the coefficient on the treatment indicator corresponds to the change in level after a single session of treatment. 
...
Schutte_g2 <- g_mlm(hlm2, p_const = c(0,0,1, B - A - 1), r_const = c(1,0,0,0,1))
Schutte_g2
```
...
[^g-mlm-mb4]: Note that the specification of the `p_const` and `r_const` arguments here differ from the specification reported in Pustejovsky, Hedges, and Shadish (2014). The `p_const` argument differs because of a small difference in the definition of the treatment-by-time interaction. The `r_const` argument appears in a different order than reported in the article because of how the variance components are arranged in the `g_mlm()` function. To see the order of the variance components used by `g_mlm()`, use the `extract_varcomp()` function: 
    ```{r}
