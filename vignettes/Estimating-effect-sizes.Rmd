...
```{r}
quality_RML <- lme(fixed = outcome ~ I(treatment=="treatment"), 
                   random = ~ 1 | case, 
...
Laski_ES_HPS <- with(Laski, effect_size_MB(outcome, treatment=="treatment", case, time))

# Pustejovsky, Hedges, & Shadish (2014)
Laski_RML <- lme(fixed = outcome ~ I(treatment=="treatment"),
                 random = ~ 1 | case, 
                 correlation = corAR1(0, ~ time | case), 
                 data = Laski)
summary(Laski_RML)
...
```{r}
Laski_RML2 <- lme(fixed = outcome ~ I(treatment=="treatment"),
                 random = ~ I(treatment=="treatment") | case, 
                 correlation = corAR1(0, ~ time | case), 
                 data = Laski)
summary(Laski_RML2)
...
Schutte$trt <- Schutte$treatment=="treatment"
Schutte$trt.week <- with(Schutte, 
                         unlist(tapply(trt * week, 
                                        list(treatment,case), 
...
```{r Model3}
hlm1 <- lme(fixed = fatigue ~ week + trt + trt.week, 
            random = ~ 1 | case, 
