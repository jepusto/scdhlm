...
Lambert <- within(Lambert, {
  case <- factor(paste("case",case))
  treatment <- factor(treatment, levels = c("SSR","RC"), ordered = TRUE)
})
str(Lambert)
...
Laski <- within(Laski, {
  case <- factor(paste("Case",case))
  treatment <- factor(treatment, levels = 0:1, labels = c("baseline","treatment"), ordered = TRUE)
})
...
Musser <- within(Musser, {
  ...
  treatment <- factor(treatment, levels = 0:2, labels = c("baseline","treatment","follow-up"), ordered = TRUE)
})
...
Schutte <- within(Schutte, {
  ...
  treatment <- factor(treatment, levels = c("baseline","treatment"), ordered = TRUE)
})
...
Thorne <- within(Thorne, {
  ...
  condition <- factor(condition, levels = 0:1, labels = c("A","B"), ordered = TRUE)
})
str(Thorne)
